...
public class JwtUtils {
    ...
    @Value("${bartu.app.jwtSecret}")
    private String jwt_secret;

    @Value("${bartu.app.jwtExpirationMs}")
    private int jwt_expiration_ms;

    public String generateJwtToken(Authentication authentication) {

        UserDetailsImpl user_principal = (UserDetailsImpl) authentication.getPrincipal();

        return Jwts.builder().setSubject((user_principal.getUsername())).setIssuedAt(new Date())
